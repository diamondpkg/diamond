...
module.exports = (pkg, options) => new Promise(async (resolve) => {
  let packages;
  const node = { nodes: [] };
  ...
  let [info, url, shasum] = [null, null, null];
  if (pkg.source.type === 'npm') {
    [info, url, shasum] = await npm(packages, pkg);
  } else if (pkg.source.type === 'github') {
    [info, url, shasum] = await github(packages, pkg);
  } else if (pkg.source.type === 'gitlab') {
    [info, url, shasum] = await gitlab(packages, pkg);
  } else if (pkg.source.type === 'diamond') {
    [info, url, shasum] = await diamond(packages, pkg);
  }
  ...
  if (old && old.for && !old.version !== pkg.version) {
    await fs.ensureDir(`./diamond/packages/${old.for}/diamond/packages`);
    await fs.rename(`./diamond/packages/${old.name}`, `./diamond/packages/${old.for}/diamond/packages/${old.name}`);
  ...
  if (pkg.for && found) {
    await fs.ensureDir(`./diamond/packages/${pkg.for}/diamond/packages`);
    await fs.remove(`./diamond/packages/${pkg.for}/diamond/packages/${pkg.name}`);
    packages.splice(index, 1);
    pkg.path = `${pkg.for}/diamond/packages/${pkg.name}`;
  } else {
    await fs.remove(`./diamond/packages/${pkg.name}`);
    if (found) {
      packages.splice(index, 1);
    }
    pkg.path = pkg.name;
  ...
  if (pkg.source.type === 'diamond' && options.cache && fs.existsSync(path.join(os.homedir(), '.diamond/package-cache', `${pkg.name}${verString}.tar.gz`))) {
    ...
    extract.on('end', async () => {
      ...
      await fs.writeFile('./diamond/.internal/packages.lock', JSON.stringify(packages));
      ...
      for (const dep of dependencies) {
        const [n] = await module.exports(dep, options);
        node.nodes.push(n);
      }
    ...
    extract.on('end', async () => {
      ...
      if (shasum && shasum !== crypto.createHash(pkg.source.type === 'diamond' ? 'sha256' : 'sha1')
        ...
        log.disableProgress();
        log.resume();
        await fs.remove(path.join('./diamond/packages'), pkg.path);
      ...
      await fs.ensureDir(path.join('./diamond/packages', pkg.path, 'diamond/dist'));
      ...
      if (/\.sass|\.scss|\.less|\.styl/.test(pkg.main)) {
        ...
        const css = await compile(path.join(process.cwd(), 'diamond/packages', pkg.path, pkg.main), { outputStyle: 'compressed' });

        await fs.writeFile(path.join('./diamond/packages', pkg.path, 'diamond/dist/main.css'), css);
        await fs.writeFile(path.join('./diamond/packages', pkg.path, 'diamond/dist/main.scss'), css);
        await fs.writeFile(path.join('./diamond/packages', pkg.path, 'diamond/dist/main.styl'), convertStylus(css));
      ...
      if (pkg.main.endsWith('.css')) {
        await fs.writeFile(path.join('./diamond/packages', pkg.path, 'diamond/dist/main.scss'), fs.readFileSync(path.join('./diamond/packages', pkg.path, pkg.main), 'utf8'));
        await fs.writeFile(path.join('./diamond/packages', pkg.path, 'diamond/dist/main.styl'), convertStylus(fs.readFileSync(path.join('./diamond/packages', pkg.path, pkg.main), 'utf8')));
      }
      ...
      if (pkg.source.type === 'diamond') {
        await fs.ensureDir(path.join(os.homedir(), '.diamond/package-cache'));
