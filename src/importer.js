...
module.exports = (file, current) => {
  ...
  if (match[2]) {
    release();
    try {
      fs.accessSync(path.join(__dirname, 'packages', pkg.name, match[2]));
    } catch (err) {
      return new Error(`could not find file '${path.join(match[1].toLowerCase(), match[2].toLowerCase())}'`);
    }
    return { file: path.join(__dirname, 'packages', pkg.name, match[2]) };
  } else if (pkg.main) {
    release();
    try {
      fs.accessSync(path.join(__dirname, 'packages', pkg.name, pkg.main));
    } catch (err) {
    ...
    return { file: path.join(__dirname, 'packages', pkg.name, pkg.main) };
  }
