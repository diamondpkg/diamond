  ...
  promise.then(data => {
    ...
    if (pkg.source.type === 'diamond' && options.cache && fs.existsSync(path.join(os.homedir(), '.diamond/package-cache', `${pkg.name}${verString}.tar.gz`))) {
      ...
      extract.on('end', () => {
        ...
        try {
          for (var _iterator2 = parsePackageObject(pkg.dependencies)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        async.eachLimit(dependencies, 1, (dep, cb) => {
          ...
          new Promise(rsolve => {
            if (/\.sass|\.scss|\.less|\.styl/.test(pkg.main)) {
              ...
              compile(path.join(process.cwd(), 'diamond/packages', pkg.path, pkg.main), { outputStyle: 'compressed' }).then(css => {
