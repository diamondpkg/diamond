...
exports.handler = () => {
  ...
  prompt.get([{ name: 'name', default: info.name || path.parse(process.cwd()).name }, { name: 'version', default: info.version || '1.0.0' }, { name: 'description', default: info.description || '' }, { name: 'main', description: 'main file', default: info.main || 'index.sass' }], (err, result) => {
    if (err) throw err;
