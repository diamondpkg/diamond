...
exports.handler = args => {
  ...
  try {
    ...
    info.main = packageJson.diamond || packageJson.sass || packageJson.less || (packageJson.main && !packageJson.main.endsWith('.js') ? packageJson.main : packageJson.style);
  ...
  stream.on('end', () => {
    superagent.post(`${config.registry}/package/${info.name}`).auth(auth.username, auth.password).field('package', JSON.stringify(info)).field('readme', readme || '').attach('dist', file, 'dist.tar.gz').then(res => {
