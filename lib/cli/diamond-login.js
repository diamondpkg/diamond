...
exports.handler = () => {
  prompt.start();
  prompt.get([{ name: 'username', required: true }], (err, result) => {
    if (err) throw err;

    superagent.get(`${config.registry}/user/${result.username}`).then(() => {
      prompt.get([{ name: 'password', required: true, hidden: true }], (e, resu) => {
        if (e) throw e;

        superagent.get(`${config.registry}/user`).auth(result.username, resu.password).then(res => {
          const auth = {
      ...
      prompt.get([{ name: 'email', required: true }, { name: 'password', required: true, hidden: true }, { name: 'verify password', required: true, hidden: true }], (e, resu) => {
        if (e) throw e;
        ...
        superagent.post(`${config.registry}/user`).send(Object.assign({}, result, resu)).then(r => {
          const auth = {
